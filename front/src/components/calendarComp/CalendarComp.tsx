...
{modalIsOpen && selectedEvent && (
        ...
        >
          <div
            className="bg-white p-6 rounded-lg max-w-md w-full overflow-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-xl font-semibold">{selectedEvent.title}</h2>
            {selectedEvent.subtitle && <h3 className="text-lg text-gray-600">{selectedEvent.subtitle}</h3>}
            {selectedEvent.description && <p className="mt-2">{selectedEvent.description}</p>}
            <p className="mt-2">Date: {new Date(selectedEvent.date).toLocaleString()}</p>
            <p className="mt-2">Location: {selectedEvent.location || 'N/A'}</p>
            <p className="mt-2">Price: ${selectedEvent.price.toFixed(2)}</p>
            <p className="mt-2">Max Seats: {selectedEvent.maxseats}</p>
